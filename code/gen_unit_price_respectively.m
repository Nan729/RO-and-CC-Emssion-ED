function [p,q,p_e,q_e,p_g,q_g]=gen_unit_price_respectively(g_type,carbon_tax)
    N=size(g_type,1);
    % heat rate data
...
%         Oil	21	123.1
    ...
    multiplier_e=zeros(1,N);
    multiplier_g=zeros(1,N);
    for i=1:N
        content=g_type(i,1);
        if contains(cell2mat(content),'Coal')
            ...
            multiplier_e(1,i)=(CO2_rate*carbon_tax)*heat_rate;
            multiplier_g(1,i)=(fuel_price)*heat_rate;
    ...
    p=p_base.*multiplier;
    q=q_base.*multiplier;
    p_e=p_base.*multiplier_e;
    q_e=q_base.*multiplier_e;
    p_g=p_base.*multiplier_g;
    q_g=q_base.*multiplier_g;
end